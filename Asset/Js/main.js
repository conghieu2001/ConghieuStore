...
for(var i=0;i<productsEvent.length;i++){
    productsEvent[i].addEventListener('mouseenter',ChooseProduct)
}
function ChooseProduct(e){
    var fvr=e.target.querySelector('.info_evaluate');
    var stars=fvr.children;
    for(var i=0;i<stars.length;i++){
        stars[i].addEventListener('click',function(e){
           if(e.target.classList.contains('evaluate__icon--liked')){
            e.target.classList.remove('evaluate__icon--liked')
           }
           else{
            e.target.classList.add('evaluate__icon--liked')
           }
        })
    }
...
function addTocartCicked(e){
    ...
    var NewProduct={
        id:parseInt(infoProduct.getElementsByClassName('info__id')[0].innerText),
        name:infoProduct.getElementsByClassName('info__name')[0].innerText,
        price:infoProduct.getElementsByClassName('info__price')[0].innerText,
        img:infoProduct.getElementsByClassName('product__img')[0].src,
        quantity:1
    }
    var Arrayproduct=[];
